...
function seotags_googlekeys($keyword) {
    $keywords = array();

	$data = wp_remote_retrieve_body( wp_remote_get('http://suggestqueries.google.com/complete/search?output=firefox&client=firefox&hl=en-US&q='.urlencode($keyword)) );
    if (($data = json_decode($data, true)) !== null) {
        $keywords = $data[1];
    }
return $keywords;
}
function seotags_bingkeys($keyword) {
    $keywords = array();
		$data = wp_remote_retrieve_body( wp_remote_get('http://api.bing.com/osjson.aspx?query='.urlencode($keyword)));
    if (($data = json_decode($data, true)) !== null) {
        $keywords = $data[1];
    }
...
function seotags_get_rand_tags() {
global $wpdb;
...
$suggests=seotags_googlekeys($tag);
		...
		{
		$google=seotags_googlekeys($suggests[$i]);
...
$suggests=seotags_bingkeys($tag);
		for($i=0;$i<count($suggests);$i++)
		{
		$google=seotags_bingkeys($suggests[$i]);
		$suggests =array_merge($suggests,$google);
		}
}
}
...
if($seotag_count=="normal")
{
$google=seotags_googlekeys($tag);
$bing=seotags_bingkeys($tag);
...
$suggests =array_merge($google,$bing);
		...
		{
		$google=seotags_googlekeys($suggests[$i]);
		$bing=seotags_bingkeys($suggests[$i]);
		$suggests =array_merge($google,$bing);
		}
}
